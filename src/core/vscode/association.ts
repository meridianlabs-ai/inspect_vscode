...
export async function withEditorAssociation(
  ...
  const configuredAssociations: Record<string, string> | undefined =
    config.get(kEditorAssociations);
  const existingAssociations =
    configuredAssociations && Object.keys(configuredAssociations).length > 0
